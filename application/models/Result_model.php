...
Class Result_model extends CI_Model
{
 ...
 function result_list($limit,$status='0',$manager_id='0',$reportee_id='0'){
	...
	if($this->input->post('search')){
	 ...
	 if($manager_id!='0')
	 {
		 $this->db->where('savsoft_result.uid',$reportee_id);
	 }
...
return $query->result_array();
 ...
 function get_result($rid,$manager_id='0'){
	$logged_in=$this->session->userdata('logged_in');
	$uid=$logged_in['uid'];

		if($logged_in['su']=='0' && $manager_id=='0'){
			$this->db->where('savsoft_result.uid',$uid);
		}
		$this->db->where('savsoft_result.rid',$rid);
	 	$this->db->join('savsoft_users','savsoft_users.uid=savsoft_result.uid');
